...
export default function SubTableProjectInfo<T>({
    data,
    //   uniqueCol,
    hide = [],
    showNumbering = true,
    ...
    const [pvmData, setPvmData] = useState<number>(0);
    const [totalData, setTotalData] = useState<number>(0);
    const [amountDataRental, setAmountDataRental] = useState<number>(0);
    const [pvmDataRental, setPvmDataRental] = useState<number>(0);
    const [totalDataRental, setTotalDataRental] = useState<number>(0);
    ...
    useEffect(() => {
        ...
        let amountRental: number = 0;
        let pvmRental: number = 0;
        let totalRental: number = 0;
        rowsToDisplay?.map((item: any) => {
            amount = amount + Number(item['Vertė']);
            pvm = pvm + (Number(item['Vertė'] * 0.21));
            total = total + (Number(item['Vertė']) * 1.21);
            amountRental = amountRental + Number(item['Visų produktų kaina D']);
            pvmRental = pvmRental + (Number(item['Visų produktų kaina D']) * 0.21);
            totalRental = totalRental + (Number(item['Visų produktų kaina D']) * 1.21);
            quantity = quantity + item.Kiekis;
        })
        setAmountData(amount);
        setPvmData(pvm);
        setTotalData(total);
        setAmountDataRental(amountRental);
        setPvmDataRental(pvmRental);
        setTotalDataRental(totalRental);
                    ...
                    <tbody>
                        ...
                        {rowsToDisplay.length > 0 &&
                            <tr className="my-2">
                                ...
                                <td>
                                    <div>
                                        <p className="m-0">{amountDataRental.toFixed(2)}</p>
                                        <p className="m-0">{pvmDataRental.toFixed(2)}</p>
                                        <p className="m-0">{totalDataRental.toFixed(2)}</p>
                                    </div>
                                </td>
                                <td>
                                    <div>
